...
module.exports = function (order) {
    ...
    // this.generateInvoice = async function (orderId, logger) {
    //     var order = await Order.findOne({ id: orderId }).exec();
    ...
    //     try {
    //         this.pdf(logger, order, currency, user, country);
    //     }
    //     catch (error) {
    //         logger.error({ message: error.message, className: 'pdfService Service' });
    //         logger.error({ message: error.stack, className: 'pdfService Service' });
    //     }
    // }
    this.order = order;
    ...
    this.generatePdfFile = function (logger, currency, user, country) {
        ...
        // Header
        content.push(
            this.adresse(this.order.idCommande, user.id, this.order.vat, new Date(), this.order.logsPayment[0].date.yyyymmdd(), this.order.id, currency.name),
            // Billing Address
            '\n',
            this.billinAddress(this.order.companyName, this.order.addressBilling, this.order.postalCodeBilling, this.order.cityBilling, this.order.countryBilling),
            '\n',
            {
                table: {
                    headerRows: 1,
    ...
    // this.pdf = function (logger, cmd, currency, user, country) {
    //     let fonts = {
    //         Roboto: {
    //             normal: new Buffer(require('pdfmake/build/vfs_fonts.js').pdfMake.vfs['Roboto-Regular.ttf'], 'base64'),
    //             bold: new Buffer(require('pdfmake/build/vfs_fonts.js').pdfMake.vfs['Roboto-Medium.ttf'], 'base64'),
    ...
    //     // Header
    //     content.push(
    //         this.adresse(cmd.idCommande, user.id, cmd.vat, new Date(), cmd.logsPayment[0].date.yyyymmdd(), cmd.id, currency.name),
    //         // Billing Address
    //         '\n',
    //         this.billinAddress(cmd.companyName, cmd.addressBilling, cmd.postalCodeBilling, cmd.cityBilling, cmd.countryBilling),
    //         '\n',
    //         {
    //             table: {
    //                 headerRows: 1,
    ...
    //                 widths: [200, 30, 65, 65, 100, 50],
    //                 margin: [0, 0, 0, 0],
    //                 body: this.getOrders(cmd.products, cmd.vatValue, style, cmd.currency, cmd.currencyTxUsd, cmd.currencyTx, country, user)
    //             },
    //             layout: { defaultBorder: false }
    //         }
    //     );
    ...
    //     invoice['header'] = header;
    //     invoice['content'] = content;
    //     let totalHT = this.priceCurrency(cmd.totalHT + cmd.totalExchangeFees, cmd.currency, cmd.currencyTxUsd, cmd.currencyTx);
    //     let total = this.priceCurrency(cmd.total, cmd.currency, cmd.currencyTxUsd, cmd.currencyTx);
    //     invoice['footer'] = this.footer(logger, totalHT, (totalHT * cmd.vatValue), total, currency, cmd.reason, cmd.vat, country, cmd.vatValide, user);

    //     // createInvoice(invoiceDetails);
    //     logger.info({ message: JSON.stringify(invoice), className: 'PDF Service' });
    //     //Création du document PDF
    //     let pdfDoc = printer.createPdfKitDocument(invoice);
    ...
    this.clientAddress = async function () {
        return request.connection.remoteAddress || request.socket.remoteAddress || request.headers['x-forwarded-for'];
    }

    this.qhAddress = async function (address, cp, city, country, sasu, rcs, vat, invoiceBillingAddress) {
        return {
            text: address + '\n' + cp + ' ' + city + '\n' + country + '\n' + sasu + '\n' + rcs + '\n' + vat,
    ...
    this.getOrders = async function (orders, vatValue, styl, currency, txUsd, tx, country) {
        let pervat = 0;
        if (country.ue === "1") {
            pervat = vatValue * 100;
        }
        ...
        if (orders.length > 0) {
            orders.forEach(order => {
                ...
                listOrders.push([
                    ...
                    { border: border, text: this.priceCurrency(order.ht, currency, txUsd, tx), margin: [0, 5, 0, 5], alignment: 'center', fontSize: 10 },
                ...
                if (order.backfill_fee > 0 || order.ongoing_fee > 0) {
                    listOrders.push([
                        ...
                        { border: border, text: "", margin: [0, 5, 0, 5], alignment: 'center', fontSize: 10 },
                        { border: border, text: "", margin: [0, 5, 0, 5], alignment: 'center', fontSize: 10 },
                        { border: border, text: this.priceCurrency((order.backfill_fee ? order.backfill_fee : order.ongoing_fee), currency, txUsd, tx), margin: [0, 5, 0, 5], alignment: 'center', fontSize: 10 },
                        { border: border, text: pervat + '%', margin: [0, 5, 0, 5], bold: true, alignment: 'center', fontSize: 10 }
                    ]);
                }
            });
    ...
    this.adresse = function (numInvoice, numAccount, idTax, invoiceDate, paymentDate, numCmd, currency) {
        return {
            columns: [
                [{ image: this.logo(), width: 200, height: 60 },
                qhAddress(
    ...
    // this.adresse = async function (numInvoice, numAccount, idTax, invoiceDate, paymentDate, numCmd, currency) {
    //     return {
    //         columns: [
    //             [{ image: this.logo(), width: 200, height: 60 },
    //             this.qhAddress(
    ...
    //             )],
    //             { text: '', width: 105 },
    //             this.head(numInvoice, numAccount, idTax, invoiceDate.toISOString().split("T")[0], paymentDate, numCmd, currency)
    //         ],
    //     }
    // };

    // this.billinAddress = async function (companyName, address, cp, city, country) {
    //     return {
    //         text: companyName + '\n' + address + '\n' + cp + ' ' + city + '\n' + country,
    //         style: 'invoiceBillingAddress'
    //     };
    ...
    // this.tabSum = async function (serviceTotal, vatTotal, invoiceTotal, currency) {
    //     // pensez à mettre les discount
    //     return {
    //         table: {
    //             widths: ['33%', '55%', '10%'],
    ...
    // this.condition = async function (conditions) {
    //     let tabCondition = [];
    //     tabCondition.push([{ text: 'General payment terms', fillColor: '#dddddd', fontSize: 8, }]);
    //     conditions.forEach(cond => {
    //         tabCondition.push([{ text: '- ' + cond, fontSize: 8, }]);
    ...
    // this.contact = async function (tel, email) {
    //     return [
    //         { text: tel, fontSize: 8 },
    //         { text: email, link: email, color: '#3a65ff', fontSize: 8 }
    //     ];
    ...
    // this.productMessage = async function (message) {
    //     if (message) {
    //         return [{ colSpan: 2, fillColor: '#dddddd', text: message, fontSize: 8 }, ''];
    //     } else {
    //         return [{ colSpan: 2, fillColor: '#FFFFFF', text: '', fontSize: 8 }, ''];
    ...
    // this.wireTransfer = async function (vat, delay, c) {
    //     return {
    //         widths: ['100%'],
    //         table: {
    //             body: [
    ...
    // this.head = async function (numInvoice, numAccount, idTax, invoiceDate, paymentDate, numCmd, currency) {
    //     let width = '50%';
    //     return {
    //         table: {
    //             headerRows: 1,
    ...
    // this.footer = async function (logger, totalHt, totalVat, totalTTC, currency, message, vat, country, vatok, user) {
    //     logger.info({ message: country, className: 'PDF Api' });
    //     let mentionvat = "";
    //     // Facture avec TVA : (client en France ou client en EU sans n° de TVA)
    //     if (country.id === 'FR' || (country.ue === '1' && !vatok)) {
    ...
    // this.logo = async function () {
    //     return `data:image/jpeg;base64,`;
    // };


    // // Fonctions Outils
    // Date.prototype.yyyymmdd = async function () {
    //     var mm = this.getMonth() + 1;
    //     var dd = this.getDate();
    //     return [this.getFullYear(), (mm > 9 ? '' : '0') + mm, (dd > 9 ? '' : '0') + dd].join('');
    // };

    // this.yyyymmdd = async function (d) {
    //     let dat = d.split('-');
    //     let mm = parseInt(dat[1]);
    //     let dd = parseInt(dat[2]);
    ...
    // this.precisionRound = async function (number, precision) {
    //     var factor = Math.pow(10, precision);
    //     return Math.round(number * factor) / factor;
    // };


    // this.priceCurrency = async function (price, currency, txUsd, tx) {

    //     if (currency !== 'usd') {
    //         return this.precisionRound((price / txUsd) * tx, 2);
    //     } else {
    //         return this.precisionRound(price, 2);
    //     }
    // };

    // this.clone = async function (obj) {
    //     try {
    //         var copy = JSON.parse(JSON.stringify(obj));
    //     } catch (ex) {
    //         alert("variable cloning error");
