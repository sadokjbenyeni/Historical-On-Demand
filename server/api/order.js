...
router.put('/delProductCaddy', async (req, res) => {
  var caddy = await OrderService.getCaddy(req.headers.authorization)
...
router.get('/caddies', async (req, res) => {
  ...
  const order = await OrderService.getCaddyv2(req.headers.authorization);
...
router.put('/updateProductDate', async (req, res) => {
  ...
  let caddy = await orderService.getCaddy(req.headers.authorization);
  if (!caddy) {
