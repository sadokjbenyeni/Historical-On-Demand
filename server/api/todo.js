...
router.put('/torun', (req, res) => {
  Order.updateOne(
    { 'products.id_undercmd'  : req.body.id_cmd },
    {
...
router.put('/finish', (req, res) => {
  let status = '';
  Order.findOne( { products: { $elemMatch: { "id_undercmd": req.body.id_cmd } } }, { 'products.$': true, _id:false  })
  .then((result)=>{
  ...
  .then(recup => {
    Order.updateOne(
      { 'products.id_undercmd'  : recup.req.id_cmd },
      {
...
router.get('/running', (req, res) => {
  Pool.find({},{_id:false, updatedAt:false, createdAt: false}).then(result => {
    res.status(200).json(result);
  })
  .catch( (err)=>{
