...
router.get('/links/:orderId', async (req, res) => {
    ...
    if (!order) return res.status(400).json({ error: "Order id not found, please contact support with '" + req.headers.loggerToken + "'" })
    if (order.idUser !== user._id) user = await Users.findOne({ _id: order.idUser }, { _id: true, roleName: true, token: true }).exec();
...
router.get('/link/:orderId/:productId', async (req, res) => {
    ...
    var order = await getOrder(req.params.orderId, user);
    if (!order || order === undefined) return res.status(400).json({ error: "Order id not found, please contact support with '" + req.headers.loggerToken + "'" })
    if (order.idUser !== user._id) user = await Users.findOne({ _id: order.idUser }, { _id: true, roleName: true, token: true }).exec();
...
async function getOrder(orderId, user) {
    if (user.roleName.indexOf('Support') != -1) return await Orders.findOne({ id: orderId }, { id: true, idUser: true, products: true, state: true, createdAt: true, id_undercmd: true }).exec();
