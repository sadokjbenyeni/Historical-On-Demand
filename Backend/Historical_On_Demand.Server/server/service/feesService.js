        ...
        order.products.forEach(item => {
            ...
            if (item.historical_data.backfill_applyfee && !(item.historical_data.direct_billing)) {
                if (item.onetime.length != 0) {
                    ...
                    orderOneTimesWithoutOverlap.forEach(item => { totalmounths += monthDiff(new Date(item.begin_date), new Date(item.end_date)) })
                }
                if (item.subscription.length != 0) totalmounths += Math.max(...item.subscription.map(element => element.period));
...
function groupProductsbyEID(order) {
    ...
    setEID.forEach(eid => {
        ...
        productsEID.forEach(item => {
            if (item.subscription == 1) subcrtiptionstab.push(item);
            else ontimetab.push(item);
        });
        mapEID.push({
            eid: eid,
...
function IsDateinRange(date, datebegining, dateend) {
    ...
    if (datebegining <= date && date <= dateend) return 0;
    if (datebegining > date) return -1;
    if (date > dateend) return 1;
}
function deleteoverlap(orderedproducts) {
    dates = [];
    while (orderedproducts.length != 0) {
        if (orderedproducts.length == 1) dates.push(orderedproducts[0]);
        else if (IsDateinRange(...orderedproducts[0].end_date, ...orderedproducts[1].begin_date, ...orderedproducts[1].end_date) == 0) orderedproducts[1].begin_date = orderedproducts[0].begin_date;
        ...
        else dates.push(orderedproducts[0]);
