...
module.exports.updateProductLogsByOrder = async (id, productsLogs) => {
    let orderToUpdate = await Orders.findOne({ id: id }).exec();
...
module.exports = function (id, logger) {
    ...
    this.addAllLogInUpdateOrder = function (orderProductLogs) {
        if (orderProductLogs === undefined) throw new Error('orderProductLogs cannot be null');
        ...
        else return this.addLogInUpdateOrder(orderProductLogs);
    }
